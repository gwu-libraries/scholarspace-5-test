...
module VttTextIndexable
  extend ActiveSupport::Concern

  # VTT files do not need the same on-disk syncing as solr does
  # we're just indexing the text, sans VTT markers

  included do
    # Override to_solr to include VTT text from attached VTT files
  end

  def to_solr
    super.tap do |index_document|
      vtt_content = extract_vtt_content
      index_document[:vtt_text] = vtt_content if vtt_content.present?
    end
  end
