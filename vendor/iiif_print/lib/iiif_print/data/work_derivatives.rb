  ...
  module Data
    ...
    class WorkDerivatives
      ...
      def commit!
        ...
        @assigned.each { |path| attach(path, path_destination_name(path)) }
      ...
      def commit_queued!(file_set)
        ...
        IngestFileRelation
          .derivatives_for_file(import_path)
          .each do |path|
      ...
      def with_io(name, &block)
        mode = %w[xml txt html].include?(name) ? "rb:UTF-8" : "rb"
        filepath = path(name)
