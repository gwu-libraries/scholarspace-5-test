...
module ScholarspaceDerivativesServices
  class ImagesToPdfDerivativesService
    ...
    def join_images_to_pdf(image_paths)
      ...
      raw_pdf_path = "#{@working_dir}/pdfs/joined_images_derivative_raw.pdf"
      @joined_pdf_path = "#{@working_dir}/pdfs/#{joined_pdf_filename}"
      return if image_paths.empty?

      cmd = ['magick', 'convert', *image_paths, raw_pdf_path]

      _stdout, _stderr, status = Open3.capture3(*cmd)
      raise 'Unable to create joined PDF from images' unless status.success?

      ensure_searchable_rendering_pdf(raw_pdf_path: raw_pdf_path, output_pdf_path: @joined_pdf_path)
    end

    def ensure_searchable_rendering_pdf(raw_pdf_path:, output_pdf_path:)
      generate_ocr_rendering_pdf!(
        source_pdf_path: raw_pdf_path,
        ocr_output_path: output_pdf_path,
        error_message: 'Unable to create OCR rendering PDF from images'
      )
    end
    ...
    def joined_pdf_filename
      'joined_images_derivative_OCR_RENDERING.pdf'
    end
    ...
    def build_joined_hocr_file(image_derivatives)
      ...
      joined_hocr_path = "#{@working_dir}/hocr/#{joined_hocr_filename}"
      combined_hocr = hocr_paths.map { |path| File.read(path) }.join("\n")
      File.write(joined_hocr_path, combined_hocr)
      joined_hocr_path
    end
