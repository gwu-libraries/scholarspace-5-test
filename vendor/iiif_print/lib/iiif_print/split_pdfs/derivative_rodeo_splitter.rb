    ...
    class DerivativeRodeoSplitter
      ...
      def initialize(filename, file_set:, output_tmp_dir: Dir.tmpdir)
        ...
        output_template_path =
          File.join(output_tmp_dir, "{{ dir_parts[-1..-1] }}", "{{ filename }}")
      ...
      def preprocessed_location_template
        if defined?(@preprocessed_location_template)
          return @preprocessed_location_template
        end

        derivative_rodeo_candidate =
          IiifPrint::DerivativeRodeoService.derivative_rodeo_uri(
            file_set: file_set,
            filename: filename
          )
        ...
        #   message =
        #     "#{self.class}##{__method__} could not establish derivative_rodeo_candidate for " \
        ...
        #   Rails.logger.debug(
        #     "#{self.class}##{__method__} found existing file at location #{derivative_rodeo_candidate}.  High five partner!"
        #   )
        #   derivative_rodeo_candidate
        # elsif file_set.import_url
        #   message =
        #     "#{self.class}##{__method__} did not find #{derivative_rodeo_candidate.inspect} to exist.  " \
        #       "Moving on to check the #{file_set.class}#import_url of #{file_set.import_url.inspect}"
        #   Rails.logger.warn(message)
        #   handle_original_file_not_in_derivative_rodeo
        # else
        #   message =
        #     "#{self.class}##{__method__} could not find an existing file at #{derivative_rodeo_candidate} " \
      ...
      def handle_original_file_not_in_derivative_rodeo
        ...
        message =
          "#{self.class}##{__method__} found #{file_set.class}#import_url of #{file_set.import_url.inspect} to exist.  " \
        ...
        # we likely have this.
        DerivativeRodeo::Generators::CopyGenerator
          .new(
      ...
      def split_files
        ...
        message =
          "#{self.class}##{__method__} encountered `#{e.class}' “#{e}” for " \
