  ...
  module FileSetLevel
    module TranscriptExtraction
      ...
      def source_file_set_ids
        av_file_sets.map { |file_set| file_set.id.to_s }
      end
      ...
      def generate_for_source_file_set_to_cache(source_file_set_id:)
        ...
        Dir.mktmpdir("audio_transcript_source_#{@work.id}_") do |dir|
          ...
          source_path = copy_single_av_to_working_dir(source_file_set)
      ...
      def persist_transcript_from_cache(source_file_set_id:, cache_file_identifier:, cache_filename:)
        return unless depositor
      ...
      def source_file_set_for(source_file_set_id)
        av_file_sets.find { |file_set| file_set.id.to_s == source_file_set_id.to_s }
      end

      def copy_single_av_to_working_dir(file_set)
        filename = file_set.original_file&.original_filename.to_s
        extension = File.extname(filename)
        extension = '.bin' if extension.blank?
        destination_path = "#{@working_dir}/av_files/#{file_set.id}#{extension}"
