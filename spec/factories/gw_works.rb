...
FactoryBot.define do
  factory :gw_work, class: "GwWork" do
    to_create do |obj|
    ...
    factory :public_gw_work, traits: [:public]
    ...
    factory :invalid_gw_work do
      title { nil }
    end

    factory :private_gw_work do
      # private is default
      # visibility { Hydra::AccessControls::AccessRight::VISIBILITY_TEXT_VALUE_PRIVATE }
    end

    factory :registered_gw_work do
      read_groups { ["registered"] }
    end

    factory :gw_work_with_one_file do
      before(:create) do |work, evaluator|
        work.ordered_members << create(
          :file_set,
          user: evaluator.user,
          title: ["A Contained Gw Work FileSet"],
          label: "gw_work_filename.pdf"
        )
      end
    end

    factory :gw_work_with_files do
      before(:create) do |work, evaluator|
        2.times do
    ...
    factory :gw_work_with_image_files do
      before(:create) do |work, evaluator|
        2.times do
          work.ordered_members << create(
            :file_set,
    ...
    factory :gw_work_with_ordered_files do
      before(:create) do |work, evaluator|
        work.ordered_members << create(:file_set, user: evaluator.user)
    ...
    factory :gw_work_with_one_child do
      before(:create) do |work, evaluator|
        work.ordered_members << create(
          :work,
          user: evaluator.user,
          title: ["A Contained Gw Work"]
        )
      end
    end

    factory :gw_work_with_two_children do
      before(:create) do |work, evaluator|
        work.ordered_members << create(
          :work,
          user: evaluator.user,
    ...
    factory :gw_work_with_representative_file do
      before(:create) do |work, evaluator|
        work.ordered_members << create(
          :file_set,
          user: evaluator.user,
    ...
    factory :gw_work_with_file_and_work do
      before(:create) do |work, evaluator|
        work.ordered_members << create(:file_set, user: evaluator.user)
        work.ordered_members << create(:gw_work, user: evaluator.user)
      end
    end

    factory :gw_work_with_embargo_date do
      # build with defaults:
      ...
      factory :embargoed_gw_work_with_files do
        after(:build) do |work, evaluator|
          if evaluator.with_embargo_attributes
            work.apply_embargo(
              evaluator.with_embargo_attributes[:embargo_date],
    ...
    factory :gw_work_with_lease_date do
      # build with defaults:
      ...
      factory :leased_gw_work_with_files do
        after(:build) do |work, evaluator|
          if evaluator.with_lease_attributes
            work.apply_lease(
              evaluator.with_lease_attributes[:lease_date],
  ...
  factory :gw_work_without_access, class: "GwWork" do
    title { ["Test title Gw Work"] }
