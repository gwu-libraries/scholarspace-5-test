module IiifPrint
  ...
  class DerivativeRodeoService
    ...
    #   @api public
    class_attribute(
      :named_derivatives_and_generators_by_type,
      default: {
        pdf: {
    ...
    class_attribute(
      ...
      default: ->(file_set:, filename:, named_derivatives_and_generators:) do
        named_derivatives_and_generators
      end
    )
    ...
    def named_derivatives_and_generators
      @named_derivatives_and_generators ||=
        if file_set.class.pdf_mime_types.include?(mime_type)
          ...
          raise UnexpectedMimeTypeError.new(
                  file_set: file_set,
                  mime_type: mime_type
                )
        end
    ...
    def self.derivative_rodeo_uri(
      file_set:,
      filename: nil,
      extension: nil,
      adapter_name: preprocessed_location_adapter_name
    )
      ...
      filename ||=
        Hydra::Works::DetermineOriginalName.call(file_set.original_file)

      dirname =
        derivative_rodeo_preprocessed_directory_for(
          file_set: file_set,
          filename: filename
        )
      ...
      location =
        DerivativeRodeo::StorageLocations::BaseLocation.load_location(
          adapter_name
        )
    ...
    def self.get_ancestor(filename: nil, file_set:)
      ...
      if DerivativeRodeo::Generators::PdfSplitGenerator.filename_for_a_derived_page_from_a_pdf?(
           filename: filename
         )
    ...
    def self.derivative_rodeo_preprocessed_directory_for(file_set:, filename:)
      ancestor, ancestor_type =
        get_ancestor(filename: filename, file_set: file_set)
      ...
      if ancestor
        message =
          "#{self.class}.#{__method__} #{file_set.class} ID=#{file_set.id} and filename: #{filename.inspect}" \
        ...
        Rails.logger.warn(
          "Expected #{ancestor.class} ID=#{ancestor.id} (#{ancestor_type} of #{file_set.class} ID=#{file_set.id}) " \
    ...
    def valid?
      if in_the_rodeo?
        Rails.logger.info(
          "Using the DerivativeRodeo for FileSet ID=#{file_set.id} with mime_type of #{mime_type}"
        )
        true
      else
        Rails.logger.info(
          "Skipping the DerivativeRodeo for FileSet ID=#{file_set.id} with mime_type of #{mime_type}"
        )
        false
      end
    end
    ...
    def create_derivatives(filename)
      named_derivatives_and_generators_filter
        .call(
          file_set: file_set,
          filename: filename,
          named_derivatives_and_generators: named_derivatives_and_generators
        )
    ...
    def cleanup_derivatives
      ...
      Hyrax::DerivativePath
        .derivatives_for_reference(file_set)
        .each { |path| FileUtils.rm_f(path) if File.exist?(path) }
    end
    ...
    def lasso_up_some_derivatives(filename:, named_derivative:, generator_name:)
      ...
      output_location_template =
        "file://#{absolute_derivative_path_for(named_derivative: named_derivative)}"
      ...
      preprocessed_location_template =
        self.class.derivative_rodeo_uri(
          file_set: file_set,
          filename: filename,
          extension: generator.output_extension
        )
      ...
      begin
        ...
        message =
          "#{generator}#generated_files encountered `#{e.class}' “#{e}” for " \
    ...
    def supported_mime_types
      ...
      named_derivatives_and_generators_by_type.keys.flat_map do |type|
        file_set.class.public_send("#{type}_mime_types")
      end
    end
    ...
    def input_uri
      ...
      location =
        DerivativeRodeo::StorageLocations::BaseLocation.from_uri(input_uri)
      @input_uri =
        if location.exist?
          input_uri
