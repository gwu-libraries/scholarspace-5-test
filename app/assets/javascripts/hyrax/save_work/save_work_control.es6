...
export default class SaveWorkControl {
  ...
  /**
   * Strip file inputs before final submit so create only sends uploaded file IDs.
   */
  preventFileBlobResubmission() {
    this.form.on('submit', () => {
      if (!this.isValid() || this.uploads.inProgress) {
        return
      }

      this.form.find('input[type="file"]').each((_idx, input) => {
  ...
  addFileUploadEventListeners() {
    const uploadsEl = this.uploads.element
  ...
  preventSubmit() {
    ...
    this.preventFileBlobResubmission()
