module Riiif
  class ImagesController < ::ApplicationController
    before_action :link_header, only: %i[show info]
    ...
    def show
      begin
        ...
        status =
          (authorization_service.can?(:show, image) ? :ok : :unauthorized)
    ...
    def info
      ...
      if authorization_service.can?(:info, image)
        ...
        unless image_info.valid?
          return render json: { error: "no info" }, status: :not_found
        end
        headers["Access-Control-Allow-Origin"] = "*"
        # Set a Cache-Control header
        expires_in cache_expires, public: public_cache?
        render json: image_info.to_h.merge(server_info),
    ...
    def server_info
      {
        ...
        PROFILE => [
          LEVEL1,
          "formats" => IIIF::Image::OptionDecoder::OUTPUT_FORMATS
        ]
      }
    end
  end
