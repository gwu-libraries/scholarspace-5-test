...
module IiifPrint
  ...
  class Engine < ::Rails::Engine
    ...
    config.to_prepare do
      ...
      if defined?(Bulkrax) &&
           !ENV.fetch("SKIP_IIIF_PRINT_BULKRAX_VERSION_REQUIREMENT", false)
      ...
      Dir
        .glob(File.join(File.dirname(__FILE__), "../../app/**/*_decorator*.rb"))
        .sort
        .each { |c| Rails.configuration.cache_classes ? require(c) : load(c) }

      Dir
        .glob(File.join(File.dirname(__FILE__), "../../lib/**/*_decorator*.rb"))
        .sort
        .each { |c| Rails.configuration.cache_classes ? require(c) : load(c) }
      ...
      Hyrax.config do |config|
        config
          .callback
          .set(:after_create_fileset) do |file_set, user|
            IiifPrint.config.handle_after_create_fileset(file_set, user)
          end

        config.simple_schema_loader_config_search_paths += [
          IiifPrint::Engine.root
        ] if config.respond_to?(:simple_schema_loader_config_search_paths)
      end
    end
