...
class ThumbnailResolver
  ...
  def representative_thumbnail_path_for_work(work:)
    ...
    thumbnail = if work.respond_to?(:thumbnail_id) && work.thumbnail_id.present?
                  find_resource(work.thumbnail_id)
                end
    thumbnail = representative_thumbnail_member_for_work(work) unless representative_thumbnail?(thumbnail)
  ...
  def representative_thumbnail_tagged_member?(member)
    DerivativeLinkResolver.related_url_values_for(member).any? { |value|
      value.start_with?(REPRESENTATIVE_THUMBNAIL_TAG_PREFIX)
    }
  end
  ...
  def representative_thumbnail?(thumb)
    ...
    return true if representative_thumbnail_tagged_member?(thumb)
  ...
  def representative_thumbnail_member_for_work(work)
    work_members(work).find { |member| representative_thumbnail?(member) }
  end

  def work_members(work)
    return Array(work.member_file_sets) if work.respond_to?(:member_file_sets)
    return [] unless work.respond_to?(:member_ids)

    Array(work.member_ids).filter_map { |member_id| find_resource(member_id) }
  end
