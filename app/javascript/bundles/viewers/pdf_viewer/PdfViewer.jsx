      ...
      <div className={styles.panel}>
        <div ref={containerRef} className={styles.documentShell}>
          <Document
            ...
            noData={<div className={styles.status}>No PDF available.</div>}
            onLoadSuccess={onDocumentLoadSuccess}
          >
            <div className={styles.pageStack}>
              {Array.from({ length: numPages || 0 }, (_item, index) => (
                <div key={`pdf-page-${index + 1}`} className={styles.pageFrame}>
                  <Page
                    pageNumber={index + 1}
                    renderAnnotationLayer
                    renderTextLayer
                    width={Math.max((containerWidth || 0) - 24, 280) || undefined}
                  />
                </div>
              ))}
            </div>
