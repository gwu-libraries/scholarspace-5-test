...
class DerivativeJobs::FileSetLevel::PresentationVersion::ImageGenerateJob < ApplicationJob
  queue_as :derivatives_presentation_version_image_generate
  ...
  def perform(work_id:, source_file_set_id:)
    with_work(work_id: work_id) do |work|
      payload = Derivatives::FileSetLevel::PresentationVersion
                .new(work)
                .generate_image_presentation_to_cache(source_file_set_id: source_file_set_id)

      next unless payload

      DerivativeJobs::FileSetLevel::PresentationVersion::ImagePersistJob.perform_later(
