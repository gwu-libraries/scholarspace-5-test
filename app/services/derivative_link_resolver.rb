...
class DerivativeLinkResolver
  ...
  def transcript_file_set?(member)
    ...
    return true if mime == VTT_MIME_TYPE

    File.extname(member_filename(member).to_s).downcase == '.vtt'
  end
  ...
  def member_filename(member)
    ...
    if member.respond_to?(:title)
      title = member.title
      return Array(title).first.to_s if title.present?
    end
  ...
  def related_url_candidates_for(member)
    ...
    if member.respond_to?(:solr_document)
      solr_document = member.solr_document
      if solr_document.respond_to?(:[])
        candidates.concat(Array(solr_document['related_url_tesim']))
      end
    end
