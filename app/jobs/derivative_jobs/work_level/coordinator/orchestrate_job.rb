...
class DerivativeJobs::WorkLevel::Coordinator::OrchestrateJob < ApplicationJob
  ...
  def has_supported_derivative_source_files?
    file_types.any? { |mime_type| supported_derivative_source_file_type?(mime_type) }
  end

  def has_pdf_source_files?
    @has_pdf_source_files ||= @work.original_member_file_sets.any?(&:pdf?)
  end

  def has_image_source_files?
    @has_image_source_files ||= @work.original_member_file_sets.any?(&:image?)
  end
  ...
  def schedule_derivatives_jobs(work_id:, retries:)
    ...
    if file_types.any? { |ft| ft.start_with?(IMAGE_MIME_PREFIX, VIDEO_MIME_PREFIX) } || has_pdf_source_files?
    ...
    if file_types.any? { |ft| ft.start_with?(*AUDIO_VIDEO_MIME_PREFIXES) }
