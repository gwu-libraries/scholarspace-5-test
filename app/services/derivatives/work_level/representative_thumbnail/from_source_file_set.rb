...
module Derivatives
  module WorkLevel
    module RepresentativeThumbnail
      class FromSourceFileSet
        ...
        def generate_to_cache
          ...
          Dir.mktmpdir("representative_thumbnail_generate_#{@work.id}_") do |dir|
            @working_dir = dir
            source_thumbnail_filename = thumbnail_filename_for(source_file_set)
            source_thumbnail_path = generate_thumbnail_asset(source_file_set, source_thumbnail_filename)
            return nil unless source_thumbnail_path && File.exist?(source_thumbnail_path)

            output_path = File.join(@working_dir, REPRESENTATIVE_THUMBNAIL_FILENAME)
            FileUtils.cp(source_thumbnail_path, output_path)
            cache_payload(source_file_set: source_file_set, output_path: output_path)
          end
        rescue StandardError => e
          Rails.logger.error("Representative thumbnail cache generation failed for work #{@work.id}: #{e.class} #{e.message}")
          raise
        end
        ...
        def persist_from_cache(source_file_set_id:, cache_file_identifier:, cache_filename:)
          ...
          Dir.mktmpdir("representative_thumbnail_persist_#{@work.id}_") do |dir|
            @working_dir = dir
            ...
            representative_thumbnail = find_service_file_set_by_filename(cache_filename)
            ...
            representative_thumbnail = tag_as_representative_thumbnail(representative_thumbnail)
          ...
          Rails.logger.error("Representative thumbnail persist failed for work #{@work.id}: #{e.class} #{e.message}")
          raise
        end
        ...
        def self.generator_class_for(file_set)
          return FileSetLevel::ThumbnailCreation::FromImage if FileSetLevel::ThumbnailCreation::FromImage.supported_file_set?(file_set)
          return FileSetLevel::ThumbnailCreation::FromPdf if FileSetLevel::ThumbnailCreation::FromPdf.supported_file_set?(file_set)
          return FileSetLevel::ThumbnailCreation::FromAudioVisual if FileSetLevel::ThumbnailCreation::FromAudioVisual.supported_file_set?(file_set)
        ...
        def best_source_file_for_thumbnail
          source_file_sets.min_by { |file_set| priority_for(file_set) }
        end
        ...
        def priority_for(file_set)
          ...
          return 2 if (file_set.respond_to?(:audio?) && file_set.audio?) ||
        ...
        def representative_cache_file_identifier_for(filename:)
          "derivatives:representative_thumbnail:work:#{@work.id}:#{filename}"
        end

        def cache_payload(source_file_set:, output_path:)
          cache_filename = REPRESENTATIVE_THUMBNAIL_FILENAME
          cache_file_identifier = representative_cache_file_identifier_for(filename: cache_filename)
        ...
        def representative_thumbnail_file_set_by_metadata
          @work.member_file_sets.find do |file_set|
            tags_for_file_set(file_set).any? { |value| value == representative_thumbnail_tag }
          end
        end
        ...
        def representative_thumbnail_tag
          "#{REPRESENTATIVE_THUMBNAIL_TAG_PREFIX}#{@work.id}"
        end
      end
    end
  end
