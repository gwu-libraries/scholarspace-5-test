  ...
  module PersistenceLayer
    class ValkyrieAdapter < AbstractAdapter
      ...
      def self.decorate_with_adapter_logic(work_type:)
        unless Hyrax.config.try(:flexible?) ||
                 work_type.included_modules.include?(
        ...
        unless Hyrax.config.try(:flexible?) ||
                 indexer.included_modules.include?(
                   Hyrax.Indexer(:child_works_from_pdf_splitting)
                 )
      ...
      def self.decorate_form_with_adapter_logic(work_type:)
        ...
        unless Hyrax.config.try(:flexible?) ||
                 form.included_modules.include?(
                   Hyrax.FormFields(:child_works_from_pdf_splitting)
                 )
      ...
      def self.parent_for(file_set)
        file_set =
          Hyrax.query_service.find_by(
            id: file_set.file_set_id
          ) if file_set.is_a?(Hyrax::FileMetadata)
      ...
      def self.destroy_children_split_from(file_set:, work:, model:, user:)
        ...
        children =
          all_child_works.select { |m| m.split_from_pdf_id == file_set.id }
        if children.blank?
          # find works where file name and work `to_param` are both in the title
          children =
            all_child_works.select do |m|
              m.title.include?(file_set.label) &&
                m.title.include?(work.to_param)
            end
        end
      ...
      def self.find_by_title_for(title:, model:)
        ...
        # maybe modify title to handle file extensions here
        Array.wrap(
      ...
      def self.index_works(objects:)
        objects.each do |work|
          ...
          Hyrax
            .custom_queries
            .find_child_file_sets(resource: work)
            .each { |file_set| Hyrax.index_adapter.save(resource: file_set) }
        end
        true
      end
      ...
      def self.extract_text_for(file_set:)
        fm =
          Hyrax.custom_queries.find_many_file_metadata_by_use(
            resource: file_set,
            use: Hyrax::FileMetadata::Use.uri_for(use: :extracted_file)
          )
        return if fm.empty?
        text_fm =
          fm.find { |t| t.mime_type == Marcel::MimeType.for(extension: "txt") }
