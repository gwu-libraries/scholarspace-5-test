...
module IiifPrint
  module WorkShowPresenterDecorator
    ...
    def iiif_viewer?
      Hyrax.config.iiif_image_server? && representative_id.present? &&
        representative_presenter.present? && members_include_viewable_image?
    end
    ...
    def members_include_viewable_image?
      ...
      #   .wrap(all_member_ids)
      #   .each do |id|
      #     if file_type_and_permissions_valid?(member_presenters_for([id]).first)
      #       return true
      #     end
      #   end
      # false
    ...
    def load_file_set_ids(solr_doc)
      solr_doc.try(:descendent_member_ids_ssim) ||
        solr_doc.try(:[], "descendent_member_ids_ssim") ||
        solr_doc.try(:member_ids_ssim) || solr_doc.try(:[], "member_ids_ssim")
    end
