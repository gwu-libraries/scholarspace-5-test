...
module Hyrax
  ...
  class FileSetDerivativesService
    ...
    def create_derivatives(filename)
      case mime_type
      when *Hyrax.config.derivative_mime_type_mappings[:pdf]
        create_pdf_derivatives(filename)
      when *Hyrax.config.derivative_mime_type_mappings[:office]
        create_office_document_derivatives(filename)
      when *Hyrax.config.derivative_mime_type_mappings[:audio]
        create_audio_derivatives(filename)
      when *Hyrax.config.derivative_mime_type_mappings[:video]
        create_video_derivatives(filename)
      when *Hyrax.config.derivative_mime_type_mappings[:image]
    ...
    def derivative_url(destination_name)
      path =
        derivative_path_factory.derivative_path_for_reference(
          derivative_url_target,
          destination_name
        )
      URI("file://#{path}").to_s
    end
    ...
    def derivative_url_target
      file_set.try(:file_set_id) ? file_set.file_set_id.to_s : file_set
    end
