...
module IiifPrint
  ...
  DEFAULT_MODEL_CONFIGURATION = {
    ...
    derivative_service_plugins:
      lambda { [IiifPrint::TextExtractionDerivativeService] }
  }.freeze
  ...
  ModelConfig =
    Struct.new(
      ...
      *DEFAULT_MODEL_CONFIGURATION.keys,
      keyword_init: true
    )
      ...
      included do
        ...
        indexer =
          if defined?(Valkyrie::Resource) && work_type < Valkyrie::Resource
            IiifPrint::PersistenceLayer::ValkyrieAdapter.decorate_with_adapter_logic(
              work_type: work_type
            )
          elsif work_type < ActiveFedora::Base
            IiifPrint::PersistenceLayer::ActiveFedoraAdapter.decorate_with_adapter_logic(
              work_type: work_type
            )
          else
        ...
        # Ensure that the work_type and corresponding indexer are properly decorated for IiifPrint
        if defined?(Valkyrie::Resource) && work_type < Valkyrie::Resource
          IiifPrint::PersistenceLayer::ValkyrieAdapter.decorate_form_with_adapter_logic(
            work_type: work_type
          )
        elsif work_type < ActiveFedora::Base
          IiifPrint::PersistenceLayer::ActiveFedoraAdapter.decorate_form_with_adapter_logic(
            work_type: work_type
          )
        else
          raise "Unable to mix '.model_configuration' into #{work_type}"
        ...
        unless indexer.respond_to?(:iiif_print_lineage_service)
          indexer.class_attribute(
            :iiif_print_lineage_service,
            default: IiifPrint::LineageService
          )
        end
        if work_type.const_defined?(:GeneratedResourceSchema)
          work_type::GeneratedResourceSchema.send(
            :include,
  ...
  def self.manifest_metadata_for(
    work:,
  ...
  def self.manifest_metadata_from(work:, presenter:)
    ...
    IiifPrint.manifest_metadata_for(
      work: work,
      current_ability: current_ability,
      base_url: base_url
    )
  ...
  def self.fields_for_allinson_flex(
    fields: allinson_flex_fields,
    ...
    fields
      .each_with_object({}) do |field, hash|
        ...
        hash[field.name] = Field.new(
          ...
          options:
            (
              if field.indexing&.include?("facetable")
                { render_as: :faceted }
              else
                nil
              end
  ...
  def self.conditionally_submit_split_for(
    work:,
    file_set:,
    locations:,
    user:,
    skip_these_endings: skip_splitting_pdf_files_that_end_with_these_texts
  )
    locations =
      locations.select do |location|
        split_for_path_suffix?(location, skip_these_endings: skip_these_endings)
      end
  ...
  def self.split_for_path_suffix?(
    path,
    skip_these_endings: skip_splitting_pdf_files_that_end_with_these_texts
  )
