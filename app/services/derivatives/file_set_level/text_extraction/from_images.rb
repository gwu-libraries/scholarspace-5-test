...
module Derivatives
  module FileSetLevel
    module TextExtraction
      # Entry point for extracting text from source images (hOCR + joined PDF flow).
      class FromImages
      include ::DerivativeFilenameConstants
      ...
      def process_image_file_set_to_cache(source_file_set_id:)
        ...
        Rails.logger.error("ImagesToPdf page cache failed for work #{@work.id}, source #{source_file_set_id}: #{e.class} #{e.message}")
        raise
      end

      def persist_page_hocr_from_cache(source_file_set_id:, cache_file_identifier:, cache_filename:)
        return unless depositor
        ...
        Rails.logger.error("ImagesToPdf persist failed for work #{@work.id}, source #{source_file_set_id}: #{e.class} #{e.message}")
        raise
      end
      ...
      def attach_page_hocr_to_work(image_derivatives)
        ...
        Rails.logger.info(
          "derivative_pipeline event=image_text_persist_batch_done work_id=#{@work.id} attached_count=#{attached_count}"
        )
      end
      ...
      def joined_pdf_file_set
        @work.member_file_sets.find do |file_set|
          ...
          attached_name == DerivativeFilenameConstants::JOINED_IMAGES_PDF_FILENAME ||
            attached_title == DerivativeFilenameConstants::JOINED_IMAGES_PDF_FILENAME
        end
      end
      ...
      def joined_hocr_filename
        JOINED_IMAGES_HOCR_FILENAME
      end

      def cache_file_identifier_for(source_file_set_id:, filename:)
        "derivatives:images_to_pdf:work:#{@work.id}:source:#{source_file_set_id}:#{filename}"
      end
    end
  end
  end
